...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        op.f("users_hashed_password_key"),
        "users",
        ["hashed_password"],
    ...
    op.add_column(
        "tasks", sa.Column("date_from", postgresql.TIMESTAMP(), autoincrement=False, nullable=True)
    )
    op.add_column("tasks", sa.Column("category", sa.INTEGER(), autoincrement=False, nullable=False))
    op.add_column(
        "tasks", sa.Column("date_to", postgresql.TIMESTAMP(), autoincrement=False, nullable=True)
    )
